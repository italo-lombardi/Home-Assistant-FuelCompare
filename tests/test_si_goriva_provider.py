...
@pytest.mark.asyncio
async def test_find_station_by_pk_found_on_page1() -> None:
    """_find_station_by_pk returns the correct station dict when found on page 1."""
    page1 = _make_mock_response(200, json_data=_search_page([_BASE_STATION]))
    session = _make_session(page1)

    provider = _default_provider()
    station = await provider._find_station_by_pk(session, _STATION_ID)

    assert station is not None
    assert station["pk"] == _STATION_PK


@pytest.mark.asyncio
async def test_find_station_by_pk_returns_none_when_not_found() -> None:
    """_find_station_by_pk returns None when pk absent from all pages."""
    other = {**_BASE_STATION, "pk": 9999}
    page1 = _make_mock_response(200, json_data=_search_page([other]))
    session = _make_session(page1)
...
@pytest.mark.asyncio
async def test_find_station_by_pk_raises_provider_error_for_non_integer() -> None:
    """_find_station_by_pk raises ProviderError for non-integer station ID."""
    session = MagicMock()
    provider = _default_provider()

    with pytest.raises(ProviderError):
        await provider._find_station_by_pk(session, "abc")


@pytest.mark.asyncio
async def test_find_station_by_pk_propagates_page1_error() -> None:
    """_find_station_by_pk propagates errors from first page."""
    page1_err = _make_mock_response(
        500,
        raise_on_raise_for_status=ClientError("error"),
    )
    session = _make_session(page1_err)

    provider = _default_provider()
    with pytest.raises(ClientError):
        await provider._find_station_by_pk(session, _STATION_ID)


@pytest.mark.asyncio
async def test_find_station_by_pk_404_returns_none() -> None:
    """_find_station_by_pk returns None when first page returns 404."""
    page1_404 = _make_mock_response(404)
    session = _make_session(page1_404)

    provider = _default_provider()
    result = await provider._find_station_by_pk(session, _STATION_ID)
